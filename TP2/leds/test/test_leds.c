...
void test_prender_y_consultar_mismo_led(void) {
  uint8_t estado=0;
  ledsTurnOnSingle(6);
  estado=ledsIsOnSingle(6);
  TEST_ASSERT_EQUAL_HEX8(1,estado);
}
// 6-Apago un led, consulto el estado y tiene que estar apagado.
...
void test_apagar_y_consultar_mismo_led(void) {
  ...
  ledsTurnOnSingle(6);
  ledsTurnOffSingle(6);
  estado=ledsIsOffSingle(6);
  TEST_ASSERT_EQUAL_HEX8(1,estado);
}
// 7-Con todos los leds apagados, prendo todos los leds y verffico que se encienden
